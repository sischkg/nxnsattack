...
public:
	...
	if ( via_tcp ) {
	    ...
	    uint16_t pos = sizeof(dns::PacketHeaderField);
		...
		question2.q_type       = query_question.q_type;
		question2.q_class      = query_question.q_class;
		question2.q_offset     = pos;
	    ...
	    answer.r_class      = query_question.q_class;
	    answer.r_ttl        = 1;
	    answer.r_resource_data = dns::ResourceDataPtr( new dns::RecordCNAME( "a" ) );
	    response.answer_section.push_back( answer );
	    /*
	    pos += answer.size();

	    dns::ResponseSectionEntry additional1;
	    additional1.r_domainname    = a;
	    additional1.r_type          = dns::TYPE_CNAME;
	    additional1.r_class         = query_question.q_class;
	    additional1.r_ttl           = 1;
	    additional1.r_resource_data = dns::ResourceDataPtr(
							       new dns::RecordCNAME( "b" ) );
	    response.additional_infomation_section.push_back( additional1 );

	    for ( unsigned int i = 0 ; i < 3000 ; i++ ) {
		dns::ResponseSectionEntry additional2;
		additional2.r_domainname    = "a";
		additional2.r_type          = dns::TYPE_CNAME;
		additional2.r_class         = dns::CLASS_IN;
		additional2.r_ttl           = 1;
		additional2.r_resource_data = dns::ResourceDataPtr(
								   new dns::RecordCNAME( "a", pos ) );
		additional2.r_offset        = pos;
...
int main( int argc, char **argv )
{
    ...
    po::options_description desc( "mutiple qname server" );
