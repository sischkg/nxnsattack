...
namespace dns
{
     ...
     **********************************************************/
    template<class T>
    std::shared_ptr<ResourceData> XNameGenerator<T>::generate( const PacketInfo &hint )
    {
        ...
        return std::shared_ptr<ResourceData>( new T( DomainnameGenerator().generate( hint_name ) ) );
    }

    template<class T>
    std::shared_ptr<ResourceData> XNameGenerator<T>::generate()
    {
        return std::shared_ptr<ResourceData>( new T( DomainnameGenerator().generate() ) );
    }
     ...
     * AGenarator
     **********************************************************/
    std::shared_ptr<ResourceData> AGenerator::generate( const PacketInfo &hint )
    {
        std::vector<std::shared_ptr<ResourceData> > record_a_list;
        for ( auto rr : hint.getAnswerSection() ) {
            if ( rr.r_type == TYPE_A ) {
	...
	return std::shared_ptr<ResourceData>( record_a_list[index]->clone() );
    }

    std::shared_ptr<ResourceData> AGenerator::generate()
    {
        return std::shared_ptr<ResourceData>( new RecordA( getRandom() ) );
    }
     ...
     * AAAAGenarator
     **********************************************************/
    std::shared_ptr<ResourceData> AAAAGenerator::generate( const PacketInfo &hint )
    {
        std::vector<std::shared_ptr<ResourceData> > record_a_list;
        for ( auto rr : hint.getAnswerSection() ) {
    ...
    std::shared_ptr<ResourceData> AAAAGenerator::generate()
    {
        std::vector<uint8_t> sin_addr = getRandomStream( 16 );
        return std::shared_ptr<ResourceData>( new RecordAAAA( &sin_addr[0] ) );
    }
     ...
     * SOAGenarator
     **********************************************************/
    std::shared_ptr<ResourceData> SOAGenerator::generate( const PacketInfo &hint )
    {
	return std::shared_ptr<ResourceData>( new RecordSOA( getDomainname( hint ),
                                                             getDomainname( hint ),
                                                             getRandom(),
                                                             getRandom(),
                                                             getRandom(),
    ...
    std::shared_ptr<ResourceData> SOAGenerator::generate()
    {
	return std::shared_ptr<ResourceData>( new RecordSOA( generateDomainname(),
                                                             generateDomainname(),
                                                             getRandom(),
                                                             getRandom(),
                                                             getRandom(),
     ...
     * RRSIGGenarator
     **********************************************************/
    std::shared_ptr<ResourceData> RRSIGGenerator::generate( const PacketInfo &hint )
    {
        std::vector<uint8_t> signature = getRandomStream( 256 );

	std::shared_ptr<ResourceData> p( new RecordRRSIG( getRandom( 0xffff ), // type covered
    ...
    std::shared_ptr<ResourceData> RRSIGGenerator::generate()
    {
        std::vector<uint8_t> signature = getRandomStream( 256 );
	return std::shared_ptr<ResourceData>( new RecordRRSIG( getRandom( 0xffff ), // type covered
                                                               getRandom( 0xff ),   // algorithm
                                                               getRandom( 0xff ),   // label
                                                               getRandom(),         // original ttl
                                                               getRandom(),         // expiration
     ...
     * DNSKEYGenarator
     **********************************************************/
    std::shared_ptr<ResourceData> DNSKEYGenerator::generate( const PacketInfo &hint )
    {
        std::vector<uint8_t> public_key = getRandomStream( 132 );
	return std::shared_ptr<ResourceData>( new RecordDNSKEY( getRandom() % 2 ? RecordDNSKEY::KSK : RecordDNSKEY::ZSK,
                                                                RecordDNSKEY::RSASHA1,
                                                                public_key ) );
    }

    std::shared_ptr<ResourceData> DNSKEYGenerator::generate()
    {
        std::vector<uint8_t> public_key = getRandomStream( 132 );
	return std::shared_ptr<ResourceData>( new RecordDNSKEY( getRandom() % 2 ? RecordDNSKEY::KSK : RecordDNSKEY::ZSK,
                                                                RecordDNSKEY::RSASHA1,
    ...
    std::shared_ptr<ResourceData> DSGenerator::generate()
    {
        if ( getRandom( 2 ) ) {
            std::vector<uint8_t> hash = getRandomStream( 40 );
            return std::shared_ptr<ResourceData>( new RecordDS( getRandom( 0xffff ),
     ...
     * NSECGenarator
     **********************************************************/
    std::shared_ptr<ResourceData> NSECGenerator::generate( const PacketInfo &hint )
    {
        ...
        return std::shared_ptr<ResourceData>( new RecordNSEC( getDomainname( hint ),
                                                              types ) );
    }

    std::shared_ptr<ResourceData> NSECGenerator::generate()
    {
        std::vector<Type> types;
        unsigned int type_count = getRandom( 0xffff );
        for ( unsigned int i = 0 ; i < type_count ; i++ ) {
    ...
    ResourceRecordGenerator::ResourceRecordGenerator()
    {
        ...
        mGenerators.push_back( std::shared_ptr<ResourceDataGeneratable>( new CNAMEGenerator ) );
        mGenerators.push_back( std::shared_ptr<ResourceDataGeneratable>( new DNAMEGenerator ) );
        mGenerators.push_back( std::shared_ptr<ResourceDataGeneratable>( new AGenerator ) );
        mGenerators.push_back( std::shared_ptr<ResourceDataGeneratable>( new AAAAGenerator ) );
        mGenerators.push_back( std::shared_ptr<ResourceDataGeneratable>( new SOAGenerator ) );
        mGenerators.push_back( std::shared_ptr<ResourceDataGeneratable>( new RRSIGGenerator ) );
    ...
    RRSet ResourceRecordGenerator::generate( const PacketInfo &hint )
    {
        ...
        std::shared_ptr<ResourceData> resource_data = mGenerators[ getRandom( mGenerators.size() )]->generate( hint );
